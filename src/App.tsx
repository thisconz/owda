...
export default function App() {
  ...
  // 3. PAGE ROUTER (Memoized to prevent unnecessary sub-tree reconciliation)
  const ActivePage = useMemo(() => {
    switch (activeTab) {
      case 'workspace': return <WorkspacePage />;
      case 'explorer': return <SimulatePage />;
  ...
  return (
    ...
    `}>
      ...
      {/* 4. ATMOSPHERIC ENGINE LAYER (Zero-Interaction Surface) */}
      ...
      {/* 5. MAIN INTERFACE FRAME */}
      <div className={`flex h-screen relative z-10 transition-all duration-700 ${error ? 'blur-md grayscale scale-[0.99]' : ''}`}>
        ...
        <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-black/20">
          ...
          <div className="flex-1 relative overflow-hidden">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="max-w-[1800px] mx-auto p-6 md:p-10 lg:p-16">
                  {ActivePage}
                </div>
              </motion.div>
            </AnimatePresence>
      ...
      {/* 6. CRITICAL SYSTEM OVERRIDE (CRASH HANDLER) */}
      <AnimatePresence>
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="flex items-start gap-6 mb-8 relative z-10">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] font-mono text-red-400/50">EXCEPTION_CODE: {error.code || '0xZ_001'}</span>
                </div>
              </div>
