...
import {
  Activity, Flame, Database, Scale,
  Trophy, RefreshCw, ChevronRight, ChevronLeft,
} from "lucide-react";
...
/**
 * Use `T | undefined` (not `T?`) throughout so that
 * exactOptionalPropertyTypes never complains about explicit undefined values.
 */
interface ReactionData {
  isBalanced:   boolean;
  reactants:    { molecules: MoleculeEntry[] };
  products:     { molecules: MoleculeEntry[] };
...
interface MoleculeEntry {
  molecule:    { formula: string; molarMass: number };
  coefficient: number;
}
...
export function ComparePage() {
  ...
  /**
   * Stable callback — reactionLog is the only external dep.
   * Now correctly listed in both useMemo dependency arrays below.
   */
  const resolveData = useCallback(
    (expr: string): ReactionData | undefined => {
      ...
      // Explicit construction avoids spread + exactOptionalPropertyTypes issues
      return {
        isBalanced:   true,
        reactants:    balanced.reactants as { molecules: MoleculeEntry[] },
        products:     balanced.products  as { molecules: MoleculeEntry[] },
  ...
  const data1 = useMemo(() => resolveData(reac1), [reac1, resolveData]);
  const data2 = useMemo(() => resolveData(reac2), [reac2, resolveData]);

  const handleSwap = () => { setReac1(reac2); setReac2(reac1); };
  ...
  return (
    ...
    >
      ...
      <header className="grid grid-cols-1 md:grid-cols-12 border-4 border-[#1A1A1A] bg-white shadow-[8px_8px_0px_#1A1A1A] overflow-hidden">
        ...
        <div className="md:col-span-3 p-6 bg-[#1A1A1A] flex items-center justify-center">
          <button
            ...
            onClick={handleSwap}
            className="flex items-center gap-3 bg-[#D4FF00] text-[#1A1A1A] px-6 py-3 font-black text-[10px] uppercase tracking-widest shadow-[4px_4px_0px_#FF6B6B] active:translate-y-1 active:shadow-none transition-all disabled:opacity-20 disabled:cursor-not-allowed"
          >
            <RefreshCw size={14} className={data1 && data2 ? "animate-spin" : ""} />
            Swap_Vectors
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <InputSlot
          label="Alpha" val={reac1} setVal={setReac1}
          data={data1} history={historyExpressions} accent="#D4FF00" dir="left"
        />
        <InputSlot
          label="Beta"  val={reac2} setVal={setReac2}
          data={data2} history={historyExpressions} accent="#FF6B6B" dir="right"
        />
      </div>
    </motion.div>
  );
...
function InputSlot({ label, val, setVal, data, history, accent, dir }: InputSlotProps) {
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-white border-4 border-[#1A1A1A] p-6 shadow-[8px_8px_0px_#1A1A1A]">
...
function ReactionDisplay({ data, slotLabel, accent }: ReactionDisplayProps) {
  if (!data) {
    return (
      <div className="h-72 flex flex-col items-center justify-center border-4 border-dashed border-[#1A1A1A]/20 bg-[#F9F9F9]">
  ...
  return (
    ...
    >
      ...
      <div className="p-8 space-y-8 flex-1">
        <div
          className="text-2xl font-mono font-black px-6 py-6 bg-[#F9F9F9] border-l-[12px]"
    ...
    <>
      {molecules.map((m, i) => (
        <span key={i}>
          ...
          <span
            dangerouslySetInnerHTML={{ __html: renderFormulaHTML(m.molecule.formula) }}
          />
  ...
  return (
    <div className="p-4 bg-white border-2 border-[#1A1A1A] group hover:bg-[#1A1A1A] hover:text-white transition-all">
      ...
      <div className="text-xl font-mono font-black">
        ...
        <span className="text-[9px] font-sans opacity-40 ml-1 group-hover:text-[#D4FF00]">
          {unit}
        </span>
      </div>
    </div>
...
function StabilityBlock({ value }: { value: number | undefined }) {
  ...
  return (
    <div className="p-6 border-2 border-[#1A1A1A] bg-[#1A1A1A] text-white shadow-[inset_0px_0px_20px_rgba(0,0,0,0.5)]">
      <div className="flex justify-between items-center mb-4">
        ...
        <span
          className={`text-[8px] px-2 py-0.5 font-black uppercase border-2 ${
            isStable
      ...
      <div className="text-4xl font-mono font-black mb-6 italic tracking-tighter">
        ...
        <span className="text-[10px] opacity-40 not-italic ml-2">kJ/mol</span>
      </div>
...
function ComparisonDelta({ d1, d2 }: { d1: ReactionData; d2: ReactionData }) {
  ...
  const gDiff = g1 - g2;
  const aWins = g1 < g2;
  const pct   = Math.abs((gDiff / (g1 || 1)) * 100).toFixed(1);
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center relative z-10">
        <div className="md:col-span-5">
          ...
          <p className="mt-4 text-[10px] font-black uppercase leading-tight max-w-xs opacity-70">
            ...
            favorability. Estimated kinetic advantage: {pct}%.
          </p>
        </div>
        ...
        <div className="md:col-span-3 bg-[#1A1A1A] text-[#D4FF00] p-6 text-center border-4 border-[#1A1A1A] shadow-[4px_4px_0px_#FFF]">
          ...
          <div className="text-3xl font-black italic">{pct}%</div>
        </div>
      </div>
    </motion.div>
  );
