...
import {
  Target, Activity, Cpu, BookOpen, Brain, Code2,
...
import { WORKSPACE_VERSION } from "../config/version";

// ---------------------------------------------------------------------------
// Mode config — module-level constant
// ---------------------------------------------------------------------------

const MODE_CONFIG = {
  human:   { icon: <BookOpen size={14} />,  label: "OBSERVATION",   border: "border-[#1A1A1A]", bg: "bg-white",       headerBg: "bg-[#D4FF00]"  },
  expert:  { icon: <Brain size={14} />,     label: "MECHANISM",     border: "border-[#1A1A1A]", bg: "bg-white",       headerBg: "bg-[#FF6B6B]"  },
  machine: { icon: <Code2 size={14} />,     label: "THERMODYNAMICS",border: "border-[#1A1A1A]", bg: "bg-[#1A1A1A]/5", headerBg: "bg-[#1A1A1A]"  },
} as const;
          ...
          >
            <div className="p-4 font-mono text-[12px] text-[#1A1A1A]">
              <ReactMarkdown
                components={{
                  strong: ({ children }) => (
                    <span className="bg-[#1A1A1A] text-white px-1 mx-0.5">{children}</span>
                  ),
                  code: ({ children }) => (
                    <code className="bg-[#D4FF00] px-1 border border-[#1A1A1A] font-bold">
                      {children}
                    </code>
                  ),
                }}
...
export function WorkspacePage() {
  ...
  const [logs, setLogs] = useState<string[]>([
    "CORE_READY",
    "AWAITING_REACTION_DATA",
  ]);

  /**
   * reactionState uses `undefined` for thermodynamic fields when not yet
   * estimated — never coerces to 0, which would corrupt the exothermic logic.
   */
  const reactionState = useMemo(() => {
    if (currentReaction?.isBalanced) {
      const { enthalpy, gibbs, entropy, activationEnergy } = currentReaction;
      return {
        isBalanced:       true  as const,
        enthalpy,                          // number | undefined — preserved
        gibbs,
        entropy,
        activationEnergy: activationEnergy ?? 40,
        isExothermic:     enthalpy !== undefined ? enthalpy < 0 : undefined,
      };
    }
    return {
      isBalanced:       false as const,
      enthalpy:         undefined,
      gibbs:            undefined,
      entropy:          undefined,
      activationEnergy: 40,
      isExothermic:     undefined,
    };
  }, [currentReaction]);
  ...
  useEffect(() => {
    if (reactionState.isBalanced) {
      ...
      const gibbsStr = reactionState.gibbs?.toFixed(2) ?? "—";
      const enthStr  = reactionState.enthalpy ?? "—";
      setLogs((p) =>
        [`[${ts}] R_SYNC: ΔG=${gibbsStr} | ΔH=${enthStr}`, ...p].slice(0, 12),
      );
    }
  }, [reactionState.isBalanced, reactionState.gibbs, reactionState.enthalpy]);

  // Chart data — undefined-safe, clamps NaN
  const chartData = useMemo(() => {
    const base  = 100;
    const dH    = reactionState.enthalpy ?? 0;
  ...
  // Stability bar width — clamp to 0–100%, show 0 when gibbs undefined
  const stabilityPercent = useMemo(() => {
    if (reactionState.gibbs === undefined) return 0;
    return Math.min(Math.abs(reactionState.gibbs), 100);
  }, [reactionState.gibbs]);
    ...
    >
      ...
      <header className="grid grid-cols-1 md:grid-cols-12 border-4 border-[#1A1A1A] bg-white shadow-[8px_8px_0px_#1A1A1A] overflow-hidden">
        <div className="md:col-span-4 p-5 flex items-center gap-4 bg-[#D4FF00] border-r-4 border-[#1A1A1A]">
          ...
          <div>
            ...
            <p className="text-[10px] font-mono font-black opacity-60">
              OWDA_WORKSPACE_V{WORKSPACE_VERSION}
            </p>
          </div>
        </div>
          ...
          <div className="h-4 w-full bg-[#EAE8E4] border-2 border-[#1A1A1A] p-0.5 overflow-hidden">
            <motion.div
              ...
              className={`h-full transition-colors ${
                reactionState.gibbs === undefined ? "bg-[#EAE8E4]" :
                reactionState.gibbs < 0          ? "bg-[#D4FF00]" : "bg-[#FF6B6B]"
              }`}
            />
          </div>
        </div>
        ...
        <div className="md:col-span-3 p-5 bg-[#1A1A1A] text-white flex flex-col justify-center items-center">
          ...
          <div className="flex gap-1 mt-2">
            {[...Array(5)].map((_, i) => (
              <div key={i} className={`w-3 h-1 ${i < 3 ? "bg-[#D4FF00]" : "bg-white/10"}`} />
            ))}
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-[#EAE8E4] border-4 border-[#1A1A1A] p-6 shadow-[4px_4px_0px_#1A1A1A]">
              ...
              <div className="space-y-5">
                ...
                <MetricBar label="Entropy_Delta" percent={reactionState.entropy !== undefined ? 85 : 0} />
              </div>
            </div>
            ...
            <div className="bg-white border-4 border-[#1A1A1A] p-5 shadow-[4px_4px_0px_#1A1A1A] relative overflow-hidden">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className={`px-2 py-0.5 border-2 border-[#1A1A1A] text-[9px] font-black ${
                  reactionState.isExothermic === undefined ? "bg-[#EAE8E4]" :
                  reactionState.isExothermic              ? "bg-[#D4FF00]" : "bg-[#FF6B6B]"
                }`}>
                  {reactionState.isExothermic === undefined ? "AWAITING_AI" :
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area
                      ...
                      strokeWidth={4}
                      fill={
                        reactionState.isExothermic === true  ? "#D4FF00" :
                        reactionState.isExothermic === false ? "#FF6B6B" : "#EAE8E4"
                      }
                      fillOpacity={1}
                    />
                    ...
                    <Tooltip
                      ...
                      cursor={{ stroke: "#1A1A1A", strokeWidth: 2 }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="grid gap-4">
              {currentSteps.length > 0 ? (
                ...
                <div className="border-4 border-dashed border-[#1A1A1A]/20 py-16 text-center">
                  <p className="text-[10px] font-black uppercase opacity-20 tracking-[0.5em]">
        ...
        <aside className="xl:col-span-4 flex flex-col gap-6">
            ...
            <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar">
              {logs.map((l, i) => (
                <div key={i} className={i === 0 ? "text-white flex gap-2" : "opacity-50 flex gap-2"}>
                  <span className="text-[#D4FF00] shrink-0">[{logs.length - i}]</span>
                  {l}
                </div>
              ))}
            </div>
          ...
          {/* Safety override */}
          <div className={`p-5 border-4 border-[#1A1A1A] shadow-[4px_4px_0px_#1A1A1A] transition-colors ${
            reactionState.isExothermic === true ? "bg-[#FF6B6B]" :
            reactionState.isExothermic === false ? "bg-[#D4FF00]" : "bg-[#EAE8E4]"
          }`}>
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold leading-tight uppercase opacity-80">
                  {reactionState.isExothermic === undefined
                    ? "NOMINAL: Awaiting thermodynamic analysis from AI engine."
...
function MoleculeStream({
  ...
  title: string;
  molecules: Array<{ molecule: { formula: string; molarMass: number }; coefficient: number }> | undefined;
  color: string;
}) {
  return (
          ...
          <div key={i} className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <span className="text-[8px] font-mono opacity-40 uppercase">
                {m.molecule.molarMass}u
              </span>
            </div>
          </div>
        ))}
        {(!molecules || molecules.length === 0) && (
          <div className="text-[9px] font-mono opacity-20 py-2 text-center uppercase tracking-widest">
            Idle_State
          </div>
        )}
      </div>
