...
export class GlobalErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    // Integrate with your logging service here (e.g., Sentry, PostHog)
  ...
  copyToClipboard = () => {
    const text = `${this.state.error?.stack}\n\nComponent Stack:\n${this.state.errorInfo?.componentStack}`;
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-[#050000] text-red-500 font-mono flex flex-col items-center justify-center p-4 sm:p-8 relative overflow-hidden selection:bg-red-500 selection:text-black">
          ...
          {/* Ambient Red Glow */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-red-600/10 via-transparent to-transparent pointer-events-none" />
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between border-b border-red-900/40 pb-6 gap-4">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h1 className="text-3xl font-black tracking-tighter uppercase italic">Kernel_Panic</h1>
              ...
              <div className="text-right hidden sm:block">
                <p className="text-[10px] text-red-900 font-bold uppercase tracking-[0.2em]">Zenthar OS v2.0.4</p>
                <p className="text-[9px] text-red-500/40 font-mono">Build_Hash: 8f2d1e_9901</p>
              </div>
            </div>
              ...
              <div className="lg:col-span-3 bg-black/40 border border-red-500/20 rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col h-[50vh]">
                <div className="bg-red-500/10 px-4 py-2 border-b border-red-500/20 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-widest">Diagnostic_Logs</span>
                  </div>
                  <button 
                    onClick={this.copyToClipboard}
                    className="text-[9px] hover:text-white transition-colors flex items-center gap-1.5 bg-red-500/20 px-2 py-1 rounded"
                  >
                    <Clipboard className="w-3 h-3" /> Copy_Trace
                  </button>
                </div>
                
                <div className="flex-1 p-6 overflow-auto font-mono text-xs sm:text-sm custom-scrollbar bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
                  <div className="text-white bg-red-600/20 px-2 py-1 rounded mb-4 inline-block">
                    {this.state.error?.name}: {this.state.error?.message}
                  </div>
                  <div className="text-red-400/80 leading-relaxed whitespace-pre font-mono text-[11px]">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="mt-auto p-4 border border-red-900/20 rounded-2xl opacity-40">
                  <p className="text-[9px] leading-tight">
                    * CRITICAL: Repeated failures indicate a logic loop in the Zenthar Engine. Please contact engineering if the reboot fails.
                  </p>
                </div>
              </div>
            </div>
            ...
            <AnimatePresence>
              {this.state.recoveryStep === 'rebooting' && (
                ...
                >
                  ...
                  <p className="text-red-600 font-mono text-sm animate-pulse uppercase tracking-[0.4em]">Reinitializing_Core...</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
